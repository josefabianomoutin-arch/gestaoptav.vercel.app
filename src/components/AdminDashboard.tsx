...
interface AdminDashboardProps {
  ...
  onUpdateInvoiceItems: (supplierCpf: string, invoiceNumber: string, items: { name: string; kg: number; value: number; lotNumber?: string; expirationDate?: string }[], barcode?: string, newInvoiceNumber?: string, newDate?: string, receiptTermNumber?: string, invoiceDate?: string, pd?: string, supplierNameHint?: string) => Promise<{ success: boolean; message?: string }>;
...
const AdminDashboard: React.FC<AdminDashboardProps> = (props) => {
  ...
  const systemAccesses = [
      ...
      { key: 'WALTER RODRIGUES JUNIOR', name: 'Diretor Walter Rodrigues Junior', default: '22121664866' },
  ...
  const renderContent = () => {
    ...
    switch (activeTab) {
      ...
      case 'schedule': return <AdminScheduleView suppliers={suppliers} thirdPartyEntries={thirdPartyEntries} onCancelDeliveries={onCancelDeliveries} onDeleteThirdPartyEntry={onDeleteThirdPartyEntry} perCapitaConfig={perCapitaConfig} onDeleteDelivery={props.onDeleteDelivery} onUpdateDelivery={props.onUpdateDelivery} onSaveInvoice={onSaveInvoice} onRegisterThirdPartyEntry={onRegisterThirdPartyEntry} onUpdateThirdPartyEntry={onUpdateThirdPartyEntry} />;
      case 'directorPerCapita': return <AdminDirectorPerCapita suppliers={suppliers} logs={props.directorWithdrawals} directorPerCapita={props.directorPerCapita} warehouseLog={warehouseLog} onRegister={props.onRegisterDirectorWithdrawal} onDelete={async (id) => { await props.onDeleteDirectorWithdrawal(id); }} />;
