...
export const DirectorPerCapitaTable: React.FC<DirectorPerCapitaTableProps> = ({
  ...
  // Current view mode inside active tab: 'form', 'history' or 'camara_fria'
  const [viewMode, setViewMode] = useState<'form' | 'history' | 'camara_fria'>(isReadOnly ? 'history' : 'form');
  ...
  useEffect(() => {
    if (currentUser?.name && !tempResponsible) {
      setTempResponsible(currentUser.name.toUpperCase());
    }
  }, [currentUser]);
  ...
  return (
    <div id="director-per-capita-panel" className="bg-white rounded-3xl shadow-xl border border-slate-100 overflow-hidden animate-fade-in">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <div className="flex bg-white rounded-xl p-1 border border-slate-200 shadow-sm w-full md:w-auto">
            ...
            <button
              ...
              className={`flex-1 md:flex-none px-4 py-2 rounded-lg text-[9px] font-black uppercase tracking-wider transition-all flex items-center justify-center gap-1.5 ${(viewMode === 'history' || viewingPastOrder || isReadOnly) && viewMode !== 'camara_fria' ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-500 hover:bg-slate-50'}`}
            >
              <History className="h-3.5 w-3.5" /> Histórico ({Object.keys(data?.[activeSubTab]?.[historyKey] || {}).length})
            </button>
            <button
              onClick={() => { setViewMode('camara_fria'); setViewingPastOrder(null); }}
              className={`flex-1 md:flex-none px-4 py-2 rounded-lg text-[9px] font-black uppercase tracking-wider transition-all flex items-center justify-center gap-1.5 ${viewMode === 'camara_fria' ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-500 hover:bg-slate-50'}`}
            >
              <Database className="h-3.5 w-3.5" /> Câmaras Frias
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="p-4 md:p-6">
        ...
        {viewMode === 'camara_fria' && (
          <div className="bg-white rounded-3xl overflow-hidden animate-fade-in space-y-6">
            {/* Sub Tab Buttons */}
            <div className="p-4 md:p-5 border-b border-gray-100 bg-zinc-900 text-white rounded-3xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 italic shrink-0">
              <div className="flex items-center gap-3">
                <div className="p-2 rounded-xl bg-white/10 text-white">
                  <Database className="h-6 w-6" />
                </div>
                <div>
                  <h2 className="text-sm font-black uppercase tracking-tighter leading-none">Controle de Câmaras Frias</h2>
              ...
              <div className="flex bg-zinc-800 p-1 rounded-xl border border-zinc-700">
                <button 
                  ...
                  className={`px-4 py-1.5 rounded-lg text-[9px] font-black uppercase transition-all whitespace-nowrap ${camaraFriaSubTab === 'temperature' ? 'bg-white text-zinc-900 shadow-md' : 'text-zinc-400 hover:text-white'}`}
                >
                ...
                <button 
                  ...
                  className={`px-4 py-1.5 rounded-lg text-[9px] font-black uppercase transition-all whitespace-nowrap ${camaraFriaSubTab === 'cleaning' ? 'bg-white text-zinc-900 shadow-md' : 'text-zinc-400 hover:text-white'}`}
                >
                  Controle de Limpeza
                </button>
              </div>
